...
broadcasts_col = db["broadcasts"]    # {_id: auto, message, channels, channel_names, sent_messages, created_at, expire_at, active}


def parse_duration(text):
    """Parses strings like '12h', '90m', '1h30m', '2h 15m' into seconds.
    Returns None if the text doesn't match any recognizable duration."""
    text = text.strip().lower().replace(" ", "")
    match = re.match(r"^(?:(\d+)h)?(?:(\d+)m)?$", text)
    if not match:
        return None
    hours_part, minutes_part = match.groups()
...
def format_duration(seconds):
    """Turns a second count back into a human readable '1h 30m' style string."""
...
async def admin_forward_setup(client, message: Message):
    ...
    if not step:
        raise ContinuePropagation  # not currently in the /addforward setup flow
    if message.text and message.text.startswith("/"):
        raise ContinuePropagation  # let other command handlers process it instead
...
async def sched_text_steps(client, message: Message):
    ...
    if step not in ("message", "time"):
        raise ContinuePropagation  # not currently in this part of the wizard
    if message.text.startswith("/"):
        raise ContinuePropagation  # let actual commands through
...
# ---------------------------------------------------------------------------
# Timed Broadcast: send one message to ALL linked channels for a limited
# time only (hours/minutes), then auto-delete it everywhere. Does NOT
# repeat daily — this is a one-time send. Can also be deleted early.
# ---------------------------------------------------------------------------
@app.on_message(filters.command("broadcast") & filters.private & admin_filter)
async def bc_start(client, message: Message):
    if channels_col.count_documents({}) == 0:
        await message.reply_text("No channels/groups linked yet. Use /addchannel first.")
        return
    set_state("bc_step", "message")
    set_state("bc_pending_message", None)
    set_state("bc_pending_duration", None)
    await message.reply_text(
        f"<b>{bold_style('New Timed Broadcast')}</b>\n\n"
        "Step 1 of 2: send me the message you want posted to ALL linked "
        "channels/groups.",
        parse_mode=ParseMode.HTML,
    )
...
@app.on_message(filters.private & filters.text & admin_filter)
async def bc_text_steps(client, message: Message):
    step = get_state("bc_step")
    if step not in ("message", "duration"):
        raise ContinuePropagation  # not currently in this part of the wizard
    if message.text.startswith("/"):
        raise ContinuePropagation  # let actual commands through

    if step == "message":
        set_state("bc_pending_message", message.text)
        set_state("bc_step", "duration")
        await message.reply_text(
            "Step 2 of 2: how long should it stay up before auto-deleting?\n\n"
            "Send it like:\n"
    ...
    if step == "duration":
        ...
        if seconds is None:
            await message.reply_text(
                "That doesn't look like a valid duration. Try formats like "
                "<code>30m</code>, <code>12h</code>, or <code>1h30m</code>.",
                parse_mode=ParseMode.HTML,
            )
            return
        set_state("bc_pending_duration", seconds)
        set_state("bc_step", "confirm")
        chan_count = channels_col.count_documents({})
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Send Now", callback_data="bc_go")]])
        await message.reply_text(
            f"Ready to send to all {chan_count} linked channel(s).\n"
            f"Will auto-delete after: {format_duration(seconds)}\n\n"
            f"Press Send Now to broadcast it.",
            reply_markup=kb,
        )
        return


@app.on_callback_query(filters.regex(r"^bc_go$"))
async def bc_go(client, query: CallbackQuery):
    if not is_bot_admin(query.from_user.id):
        await query.answer("Only an admin can do this.", show_alert=True)
        return

    msg_text = get_state("bc_pending_message")
    duration = get_state("bc_pending_duration")
    if not msg_text or not duration:
        await query.answer("Something's missing — please start over with /broadcast.", show_alert=True)
        return
    ...
    for c in chans:
        chat_id = c["_id"]
        names[str(chat_id)] = c.get("name", str(chat_id))
        try:
            sent = await client.send_message(chat_id, msg_text)
            sent_messages[str(chat_id)] = sent.id
        except FloodWait as e:
            await asyncio.sleep(e.value)
            try:
                sent = await client.send_message(chat_id, msg_text)
                ...
                logger.error(f"Broadcast send failed for {chat_id}: {e2}")
                failed += 1
        except Exception as e:
            logger.error(f"Broadcast send failed for {chat_id}: {e}")
            failed += 1

    now = time.time()
    broadcast_id = f"bc_{int(now)}"
    broadcasts_col.insert_one({
        "_id": broadcast_id,
        "message": msg_text,
        "channels": [c["_id"] for c in chans],
        "channel_names": names,
    ...
    set_state("bc_step", None)
    set_state("bc_pending_message", None)
    set_state("bc_pending_duration", None)

    await query.answer("Broadcast sent!")
    await query.message.edit_text(
        f"✅ <b>Broadcast sent</b> (ID: <code>{broadcast_id}</code>)\n\n"
        f"Delivered to {len(sent_messages)} channel(s)"
        + (f", {failed} failed" if failed else "")
        + f".\nWill auto-delete after {format_duration(duration)}.\n\n"
        f"Want to remove it early? Use:\n<code>/cancelbroadcast {broadcast_id}</code>",
        parse_mode=ParseMode.HTML,
    )


@app.on_message(filters.command("listbroadcasts") & filters.private & admin_filter)
async def bc_list(client, message: Message):
    broadcasts = list(broadcasts_col.find({"active": True}))
    if not broadcasts:
        await message.reply_text("No active broadcasts right now. Use /broadcast to send one.")
        return
    now = time.time()
    lines = []
    for b in broadcasts:
        remaining = b["expire_at"] - now
        preview = b["message"][:40] + ("..." if len(b["message"]) > 40 else "")
        remaining_str = format_duration(remaining) + " left" if remaining > 0 else "deleting soon"
        lines.append(
            f"<code>{b['_id']}</code> — {remaining_str}\n"
            f"  \"{preview}\" → {len(b['channels'])} channel(s)"
        )
    await message.reply_text(
        f"<b>{bold_style('Active Broadcasts')}</b>\n\n" + "\n\n".join(lines)
        + "\n\nUse /cancelbroadcast <id> to delete one early.",
        parse_mode=ParseMode.HTML,
    )


async def _delete_broadcast_everywhere(client: Client, broadcast: dict):
    for chat_id_str, msg_id in broadcast.get("sent_messages", {}).items():
        try:
            await client.delete_messages(int(chat_id_str), msg_id)
        except Exception as e:
            logger.warning(f"Couldn't delete broadcast message in {chat_id_str}: {e}")


@app.on_message(filters.command("cancelbroadcast") & filters.private & admin_filter)
async def bc_cancel(client, message: Message):
    parts = message.text.split(" ", 1)
    if len(parts) < 2 or not parts[1].strip():
        await message.reply_text("Usage: /cancelbroadcast <broadcast_id>\n\nSee IDs with /listbroadcasts.")
        return
    broadcast_id = parts[1].strip()
    broadcast = broadcasts_col.find_one({"_id": broadcast_id, "active": True})
    if not broadcast:
        await message.reply_text("No active broadcast found with that ID.")
        return

    await _delete_broadcast_everywhere(client, broadcast)
    broadcasts_col.update_one({"_id": broadcast_id}, {"$set": {"active": False}})
    await message.reply_text(f"✅ Broadcast <code>{broadcast_id}</code> deleted from all channels early.", parse_mode=ParseMode.HTML)


async def broadcast_expiry_loop(client: Client):
    """Runs forever in the background, checking once every 30s whether any
    active broadcast has passed its expiry time. When it has: delete the
    message from every channel it was sent to and mark it inactive."""
    while True:
        try:
            now = time.time()
            for broadcast in broadcasts_col.find({"active": True, "expire_at": {"$lte": now}}):
                await _delete_broadcast_everywhere(client, broadcast)
                broadcasts_col.update_one({"_id": broadcast["_id"]}, {"$set": {"active": False}})
        except Exception as e:
            logger.error(f"Broadcast expiry loop error: {e}")
...
async def admin_cancel(client, message: Message):
    set_state("awaiting_channel_link", False)
    set_state("fwd_step", None)
    set_state("sched_step", None)
    set_state("sched_editing_id", None)
    ...
    set_state("bc_pending_message", None)
    set_state("bc_pending_duration", None)
    await message.reply_text("✅ Cancelled — any in-progress setup has been cleared.")
...
async def admin_start(client, message: Message):
    text = (
        ...
        f"<b>{bold_style('Timed Broadcast')}</b>\n"
        "/broadcast - Send one message to ALL linked channels for a set "
        "duration (hours/minutes), auto-deletes after — one-time, not daily\n"
        "/listbroadcasts - Show active broadcasts with time remaining\n"
        "/cancelbroadcast &lt;id&gt; - Delete a broadcast from all channels early"
    )
...
async def main():
    ...
    asyncio.create_task(broadcast_expiry_loop(app))
