...
async def admin_addforward_start(client, message: Message):
    ...
    set_state("fwd_pending_source", None)
    await message.reply_text(
        f"<b>{bold_style('Add Forward Pair')}</b>\n\n"
        "Step 1 of 2: forward a message from the SOURCE channel "
        "(where you upload the original content)."
    )
...
async def admin_forward_setup(client, message: Message):
    ...
    if step == "source":
        set_state("fwd_pending_source", chat.id)
        set_state("fwd_step", "destination")
        await message.reply_text(
            f"Source set: {chat.title}\n\n"
            f"Step 2 of 2: forward a message from the DESTINATION — this can be "
            f"either a channel or a Topics-enabled group."
        )
        return

    if step == "destination":
        source_id = get_state("fwd_pending_source")
        dest_type = "group" if chat.type in (ChatType.SUPERGROUP, ChatType.GROUP) else "channel"
        pair_id = f"{source_id}_{chat.id}"
        forward_pairs_col.update_one(
            {"_id": pair_id},
            {"$set": {
                "source": source_id, "destination": chat.id,
                "dest_type": dest_type, "dest_name": chat.title,
            }},
            upsert=True,
        )
        ...
        note = (
            "\n\nMake sure Topics is turned on in this group's settings, and "
            "the bot has Manage Topics permission." if dest_type == "group" else ""
        )
        await message.reply_text(
            f"✅ Forward pair created.\n\nDestination: {chat.title}\nMode: {mode_label}{note}"
        )
        return
